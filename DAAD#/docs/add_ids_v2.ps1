# Script mejorado para agregar IDs
$condacts = @{
...
# Leer el archivo original (sin IDs duplicados)
$content = Get-Content "DAAD_CondActs_Reference.md"
...
foreach ($line in $content) {
    ...
    # Buscar líneas que empiecen con ### seguido de un CondAct
    if ($line -match '^### ([A-Z][A-Z0-9]*)\s*(.*)$') {
        $condactName = $matches[1]
        $rest = $matches[2]
        
        if ($condacts.ContainsKey($condactName)) {
            $id = $condacts[$condactName]
            $newLine = "### $condactName (ID: $id) $rest"
        }
    }
...
# Guardar archivo
$newContent | Set-Content "DAAD_CondActs_Reference_with_IDs_v2.md" -Encoding UTF8
Write-Host "Archivo procesado correctamente: DAAD_CondActs_Reference_with_IDs_v2.md"
